    ...
    QWidget(parent), m_pixmap(p), m_eventFilter(nullptr)
{
    ...
    colorForm->addRow(QStringLiteral("粗细:"), m_thicknessSlider);
    colorForm->addRow(QStringLiteral("颜色:"), m_colorLabel);
...
void SidePanelWidget::updateGrabButton(const bool activated) {
    if (activated) {
        m_colorGrabButton->setText(QStringLiteral("按ESC取消"));
    } else {
        m_colorGrabButton->setText(QStringLiteral("抓取颜色"));
    }
}
